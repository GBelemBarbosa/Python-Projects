...
class GUI: 
        ...
        def resourcepaste(self, num):
            index=re.search("](?!.*])", self.block_entry.get()).start()
            self.block_entry.focus()
        ...
        def antepaste(self):
            self.block_entry.insert(self.block_entry.index(INSERT), "("+str(int(int(self.ac.get())+int(self.ad[0].get())*(int(self.ad[1].get())+1)/2))+","+self.aa.get()+")[]")
            self.block_entry.focus()
            self.block_entry.icursor(self.block_entry.index(INSERT)-1)
        
        def antepaste2(self, num1, num2):
            if type(num2)!=str:
                num2=num2.get()
            bloco_text=self.block_entry.get()
            
            bloco_text=bloco_text.replace(" ", "")
            try:
                pre=re.search("\(.+?\)", bloco_text)
                bloco_text="("+str(int(re.search("-?\d+", pre.group()).group())+int(int(num1.get())*(int(num2)+1)/2))+","+re.search("(?<=,)-?\d+", pre.group()).group()+")"+bloco_text[pre.end():len(bloco_text)]
        ...
        def anteadvpaste(self, adv):
            ...
            try:
                pre=re.search("\(.+?\)", bloco_text)
                bloco_text="("+re.search("-?\d+", pre.group()).group()+","+str(int(re.search("(?<=,)-?\d+", pre.group()).group())+int(adv.get()))+")"+bloco_text[pre.end():len(bloco_text)]
                self.block_entry.delete(0, END)
                self.block_entry.insert(END, bloco_text)
                self.block_entry.icursor(self.block_entry.index(INSERT)-1)
            except:
        ...
        def layout(self,name): 
                ...
                self.antebar.place(relwidth=1,relheight=0.23,rely=0.05)
                ...
                self.antebtt = Button(self.antebar, 
                                                            text = "New block", 
                                                            font = "Courier 10 bold", 
                                                            width = 9, 
                                                            bg = 'black',
                                                            fg="white",
                                                            command = lambda : self.antepaste()) 

                self.antebtt.pack(side='bottom')
                ...
                self.aconbtt = Button(self.antebar, 
                                                            ...
                                                            bg = 'black',
                                                            fg="white",
                                                            command = lambda : self.antepaste2(self.ac, "1")) 
                ...
                self.adicbtt = Button(self.antebar, 
                                                            ...
                                                            bg = 'black',
                                                            fg="white",
                                                            command = lambda : self.antepaste2(self.ad[0], self.ad[1])) 
        ...
        def conversao(self, bloco_text):
            ...
            try:
                ...
                pre=re.search("(?<=\().+?(?=\))", bloco_text).group().split(",")
                rec=bloco((int(pre[0]),int(pre[1])),[],self.sn.get())
                pro=re.search("\[?.+\]", bloco_text).group()
                recursos=re.findall("(?<!^)\(.+?\D\)", pro)
                rec.posmod=[]
                for i in recursos:
